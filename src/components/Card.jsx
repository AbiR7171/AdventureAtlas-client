...
const Card = ({ tour, refetch }) => {
  console.log(tour);

  const { tourName, price, startDate, endDate, members, _id, image } = tour;
  ...
  return (
    <div>
      <div className="6main-font md:w-[300px] mx-auto space-y-8  my-20 relative p-8 bg-transparent  shadow-xl  rounded-md">
        {/* top part  */}

        <img
          className="w-[150px] border h-[150px] object-cover rounded-full p-2 bg-slate-100 duration-300 hover:scale-105"
          src={`https://adventure-atlas-server.vercel.app/images/`+image}
          alt=""
        />
        ...
        <div>
          ...
          <div className="absolute -top-4 -right-[20px] ">
            <div className="w-full h-full relative">
              ...
              >
                <defs>
                  ...
                  >
                    <stop
                      offset="0%"
                      style={{ stopColor: "#BE31D3", stopOpacity: 1 }}
                    />
                    <stop
                      offset="100%"
              ...
              {/* Price  */}
              <div className="absolute top-8 left-7 text-white text-xl font-semibold flex flex-col flex-wrap">
        ...
        <div className="space-y-4">
          <h3 className="text-2xl font-bold text-[#BE31D3]">{tourName}</h3>
          <h3 className="text-black">
            {startDate} - {endDate}
          </h3>
          <p className="capitalize text-black">Members</p>
          ...
          <ul className="space-y-3">
            {members?.map((m) => {
              return (
                <li
                  key={m._id}
                  className="flex items-center gap-2 text-sm text-sky-400 font-semibold"
                >
                  <svg
                    width={20}
                    viewBox="0 0 48 48"
          ...
          <div className="pt-4 flex items-center gap-2 ">
            <Link to={`/main/single-tour/${_id}`}>
              <button className="bg-black text-white p-2 px-6 flex items-center gap-2 bg-opacity-85">
                View{" "}
                <Icon icon="lets-icons:view-duotone" className="text-2xl" />
              </button>
            </Link>

            <Link to={`/main/edit-tour/${_id}`}>
