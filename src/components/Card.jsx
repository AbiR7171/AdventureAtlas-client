...
const Card = ({tour}) => {
    ...
    const {tourName,price, startDate, endDate, members} =tour;
    return (
        <div className="main-font md:w-[300px] mx-auto space-y-8 shadow-[0px_0px_30px_2px_rgba(100,100,111,0.1)] my-20 relative p-8 bg-gradient-to-r from-rose-500 to-rose-800 rounded-sm">
            {/* top part  */}
            <div>
            ...
            <h3 className="text-2xl font-bold text-sky-300">{tourName}</h3>
            <p className="capitalize text-sky-100">Members</p>
                <ul className="space-y-3">
                  {
                     members?.map(m => {
                         return  <li key={m._id} className="flex items-center gap-2 text-sm text-sky-400 font-semibold">
                ...
                <div className="pt-4 flex justify-center">
                    <div className="w-full">
                         <Button className="w-full flex items-center">view<Icon icon="lets-icons:view-duotone" /></Button>
                    </div>
                </div>
            </div>
        </div>
